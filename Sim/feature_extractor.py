...
from sim_config import (
    ...
    MIN_DATA_CONFIDENCE, RECENCY_HALF_LIFE_DAYS,
)
...
# Tasmanian tracks for mainland-visitor detection
_TAS_TRACKS = {
    'Hobart', 'Launceston', 'Burnie', 'Carrick',
    'Scottsdale', 'Spreyton', 'St Marys', 'Devonport', 'King Island',
}

# Interstate class → implied NR mapping for mainland visitors
_INTERSTATE_CLASS_NR = {
    'g1': 95, 'group 1': 95,
    'g2': 85, 'group 2': 85,
    'g3': 75, 'group 3': 75,
    'listed': 65,
...
# Positive gear changes signal horse improvement
_POSITIVE_GEAR = {'blinkers', 'hood', 'winkers', 'shadow roll'}
# Reactive gear signals a problem
_REACTIVE_GEAR = {'lugging pole', 'headcheck', 'tongue tie', 'check'}


def decay_weight(days_ago: float, half_life_days: float = None) -> float:
    """Exponential decay: runs from half_life_days ago are worth half a recent run."""
...
def compute_data_confidence(
    ...
    """Overall data confidence for a runner, 0.0–1.0.

    < MIN_DATA_CONFIDENCE (0.4) → THIN DATA warning emitted by caller.
    """
    all_runs  = _recent_runs(runner, stride_results, 200)
    n_runs    = len(all_runs)
    ...
    # Local track runs
    n_local = 0
    if track and not all_runs.empty and 'Track' in all_runs.columns:
        n_local = int((all_runs['Track'].str.lower().str.strip() == track.lower()).sum())

    base = min(1.0, n_runs / 30.0)
    if n_local < 3:
        return max(0.30, base * 0.6)
    return base


def is_mainland_visitor(runner: Runner, stride_results: pd.DataFrame) -> bool:
    """True if horse has fewer than 3 Tasmanian race results."""
    all_runs = _recent_runs(runner, stride_results, 200)
    if all_runs.empty or 'Track' not in all_runs.columns:
        return True   # No data at all → treat as visitor
    local = all_runs[all_runs['Track'].isin(_TAS_TRACKS)]
    return len(local) < 3


def estimate_nr_from_interstate(runner: Runner, stride_results: pd.DataFrame) -> float:
    """For mainland visitors with NR=0, estimate implied NR from interstate class.

    Tasmanian NR typically runs ~5 points below equivalent mainland grade.
    """
    if runner.nr > 0:
        return runner.nr

    runs = _recent_runs(runner, stride_results, 20)
    if runs.empty or 'Class' not in runs.columns:
        return 0.0

    for _, row in runs.iterrows():
        cls = str(row.get('Class', '')).lower().strip()
        for key, implied_nr in _INTERSTATE_CLASS_NR.items():
            if key in cls:
                return float(implied_nr) - 5.0   # Tasmania discount
    return 0.0


def check_odm_mobile(
    runner: Runner,
    stewards_notes: pd.DataFrame,
    distance_m: int = 0,
) -> float:
    """Returns ODM penalty multiplier (1.0 = normal, <1.0 = penalised).

    Checks runner.odm_mobile flag first, then stewards_notes for 'out of the draw'.
    Penalty depends on distance: short races are more punishing.
    """
    is_odm = getattr(runner, 'odm_mobile', False)

    if not is_odm and not stewards_notes.empty:
        # Also check stewards notes for historical ODM events
        slug, slug_nz, name_norm, name_norm_nz = _horse_lookup_keys(runner)
        if 'Horse_Slug' in stewards_notes.columns:
            mask = stewards_notes['Horse_Slug'].str.lower().str.strip() == slug
        elif 'Horse' in stewards_notes.columns:
            mask = stewards_notes['Horse'].str.lower().str.strip() == name_norm
        else:
            mask = pd.Series(False, index=stewards_notes.index)

        horse_notes = stewards_notes[mask]
        for _, row in horse_notes.head(5).iterrows():
            note = str(row.get('Note', '')).lower()
            if 'out of the draw' in note or 'out of draw' in note or 'odm' in note:
                is_odm = True
                break
    ...
    # Distance-based penalty: shorter = more punishing
    if distance_m > 0 and distance_m >= 2200:
        return 0.60   # Longer race — more time to recover
    return 0.30       # Short race (<2000m) — severe
...
def check_false_start(
    ...
    """True if a false start is noted in stewards for this specific race.

    False starts corrupt sectional data — gate_speed and position_800m_margin
    features should be zeroed when this returns True.
    """
    if stewards_notes.empty:
        return False

    false_start_keywords = ('false start', 'false barrier', 'recall', 'restarted')
    race_date_lower = str(race_date).strip()

    for _, row in stewards_notes.iterrows():
        row_date = str(row.get('Date', '')).strip()
        row_track = str(row.get('Track', '')).strip().lower()
        note = str(row.get('Note', '')).lower()

        # Match by date and track; race_no match if column exists
        date_match = race_date_lower in row_date or row_date in race_date_lower
        track_match = (not row_track) or (track.lower() in row_track)
        note_match = any(kw in note for kw in false_start_keywords)

        if date_match and track_match and note_match:
            return True

    return False


def detect_trainer_change(runner: Runner, stride_results: pd.DataFrame) -> float:
    """Trainer change signal.

    Returns +0.1 if current trainer differs from trainer in last 3 results.
    Returns +0.2 if new trainer has higher win% in stride_results than previous.
    Returns 0.0 if no change or insufficient data.
    """
    ...
    prev_trainer = recent_trainers[0]
    if current_trainer == prev_trainer:
        return 0.0   # No change

    # Trainer changed — check if new trainer has better win% in stride_results
    if stride_results.empty or 'Trainer' not in stride_results.columns:
        return 0.1

    def _trainer_winrate(name: str) -> float:
        mask = stride_results['Trainer'].str.lower().str.strip() == name
        rows = stride_results[mask]
        if rows.empty:
            return 0.0
        wins = (pd.to_numeric(rows.get('Place', pd.Series(dtype=float)), errors='coerce') == 1).sum()
        return float(wins / len(rows))

    new_rate  = _trainer_winrate(current_trainer)
    prev_rate = _trainer_winrate(prev_trainer)

    return 0.20 if new_rate > prev_rate else 0.10


def compute_gear_change_signal(runner: Runner) -> float:
    """First-time gear change signal from runner.gear_changes list.

    Requires form_parser.py to have captured gear changes (currently partial support).
    Returns +0.15 for positive gear, -0.1 for reactive gear, 0.0 if no change.
    """
    changes = getattr(runner, 'gear_changes', [])
    if not changes:
        return 0.0
    ...
    for change in changes:
        chg = change.lower()
        if any(g in chg for g in _POSITIVE_GEAR) and 'on' in chg:
            score += 0.15
        elif any(g in chg for g in _REACTIVE_GEAR):
            score -= 0.10

    return float(np.clip(score, -0.30, 0.30))
...
def compute_head_to_head(
    ...
    for opponent in field:
        if opponent.slug == runner.slug:
            continue
        opp_runs = _recent_runs(opponent, stride_results, 100)
        if opp_runs.empty:
            continue
...
def check_feature_spread(all_features: Dict[str, Dict[str, float]], slugs: List[str]) -> dict:
    """Diagnostic: compute std-dev of each feature across the field.

    Features with std < 0.01 are 'FLAT — not differentiating'.
    Returns {feature: std_value}.
    """
    from race_sim import SCORED_FEATURES  # avoid circular at module level
    spreads = {}
    flat_features = []
    ...
    if len(flat_features) > 3:
        print(f"[WARN] {len(flat_features)} features are FLAT (std < 0.01): {flat_features}")
    elif flat_features:
        for f in flat_features:
            print(f"[WARN] Feature '{f}' is FLAT — not differentiating field")
...
def extract_features(
    ...
    # Data confidence score (used as multiplier on all feature values below)
    ...
    if is_visitor:
        data_conf = min(data_conf, 0.30)  # Hard cap confidence for visitors
        warnings.append(
            f"{runner.horse}: MAINLAND VISITOR — Tasmania data insufficient "
            f"(confidence discounted to {data_conf:.0%})"
        )
        # Try to estimate NR from interstate class if NR is zero
        if runner.nr == 0:
            implied_nr = estimate_nr_from_interstate(runner, data.stride_results)
            if implied_nr > 0:
                warnings.append(
                    f"{runner.horse}: using implied NR {implied_nr:.0f} from interstate class"
                )

    # Emit THIN DATA warning below confidence threshold
    if data_conf < MIN_DATA_CONFIDENCE:
        warnings.append(
            f"{runner.horse}: THIN DATA — confidence {data_conf:.0%} "
            f"(< {MIN_DATA_CONFIDENCE:.0%})"
        )

    # False start check for this race (zeros gate_speed and position_800m_margin)
    ...
    feats['head_to_head'] = h2h if isinstance(h2h, float) else 0.0

    # Trainer change signal
    feats['trainer_change'] = detect_trainer_change(runner, data.stride_results)

    # First-time gear change signal
    feats['gear_change_signal'] = compute_gear_change_signal(runner)

    # ── Apply false start: zero corrupted sectional features ─────────────────
    if false_start:
        warnings.append(f"FALSE START detected for {track} race — gate_speed zeroed")
        feats['gate_speed'] = 0.0
        feats['position_800m_margin'] = 0.0

    # ── Apply data confidence multiplier to all scorable features ────────────
    # Private features prefixed with '_' are not scaled (they're meta-data).
    # gate_speed and finishing_speed are raw absolute values (not near zero),
    # so scaling toward zero distorts their relative field position when z-scored.
    # These features are left at their raw values; data confidence is expressed
    # through the DLW decay and other downstream attenuation.
    _RAW_ABSOLUTE_FEATURES = frozenset({'gate_speed', 'finishing_speed'})
    if data_conf < 1.0:
        for key in list(feats.keys()):
            if key.startswith('_'):
                continue
            if key in ('freshness', 'injury_return', 'driver_suspended', 'stewards_flag'):
                continue  # These are categorical flags, don't scale
            if key in _RAW_ABSOLUTE_FEATURES:
                continue  # Raw absolute values — scaling toward zero distorts z-scores
            feats[key] = feats[key] * data_conf

    # ── Field strength — same scalar for all runners ──────────────────────────
    field_strength = compute_field_strength(field, data.stride_profiles)
    feats['field_strength'] = field_strength   # not confidence-scaled
    ...
    feats['class_relativity'] = cr * data_conf
    warnings.extend(cw)
