...
# Burnie gate-style bases: Modern Jive is the pace-setter (small margin),
# Andaman Bay is a class closer (settles mid-field despite high NR).
# Finishing speed (400_0_Time) still correlates with NR — class tells in the straight.
_BURNIE_GATE_800M = {
    'modern-jive': 0.6,   # Front-runner — consistently close to leader
    'nikita-jo':   1.4,   # Average gate
    'andaman-bay': 1.2,   # Settles; class tells in the sprint, not the gate
    'la-pierre':   1.6,   # Below-average gate
    'rock-amour':  1.8,   # Slow out
    'away-game':   1.9,   # Slowest
}
...
for display, slug, barrier, driver, nr, n_runs, _wp in BURNIE_HORSES:
    ...
    for i, date in enumerate(dates):
        ...
        # Gate-style based 800m margin (not purely NR-driven)
        base_800m  = max(0.2, _BURNIE_GATE_800M.get(slug, 1.5) + random.uniform(-0.4, 0.4))
        ...
        stride_rows.append({
            ...
            "Last_800m_Pos": max(1, place - 1 + random.randint(-1, 2)),
            "400_0_Time":    round(29.2 - (nr - 45) * 0.12 + random.uniform(-0.2, 0.2), 2),
            "Mile_Rate":     round(mile_rate - trend * 0.1, 2),
...
print(f"[OK] stride_results.csv   — {len(sr_df)} rows, {sr_df['Slug'].nunique()} horses")
...
print(f"[OK] stride_profiles.csv  — {len(sp_df)} rows ({sp_df['Slug'].nunique()} horses)")
...
print(f"[OK] drivers_profile.csv  — {len(driver_rows)} drivers")
...
print(f"[OK] driver_results_recent.csv — {len(dr_rows)} rows")
...
print(f"[OK] trainer_results_recent.csv — {len(tr_rows)} rows")
...
print(f"[OK] stewards_notes.csv   — {len(sn_rows)} rows")
...
print(f"[OK] dividends.csv        — {len(div_rows)} rows")

print(f"\n[OK] Full dataset written to {OUT}/")
