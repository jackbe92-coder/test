...
class DataLoader:
    ...
    def _enrich_stride_results(self):
        """Join dividends Start_Type and Track_Condition onto stride_results."""
        if self.stride_results.empty or self.dividends.empty:
            return
        cols_needed = [c for c in ('Date', 'Track', 'Race_No', 'Start_Type', 'Track_Condition')
                       if c in self.dividends.columns]
        if not all(c in cols_needed for c in ('Date', 'Track', 'Race_No')):
            return
        div_sub = self.dividends[cols_needed].drop_duplicates(subset=['Date', 'Track', 'Race_No'])
        # Avoid duplicate columns if already present
        for col in ('Start_Type', 'Track_Condition'):
            if col in self.stride_results.columns:
                self.stride_results = self.stride_results.drop(columns=[col])
        try:
            self.stride_results = self.stride_results.merge(
