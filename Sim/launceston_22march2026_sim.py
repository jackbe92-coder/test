...
DATA  = r"C:\Users\evil_\Desktop\Scraper\output\claude_data"
RUNS  = 2000
TRACK = "Launceston"
...
def data_coverage(race_info, data):
    ...
    sr_slugs = set(data.stride_results['_slug'].tolist()) if '_slug' in data.stride_results.columns else set()
    def _found(slug):
        if slug in sr_slugs:
            return True
        nz = slug if slug.endswith('-nz') else slug + '-nz'
        return nz in sr_slugs
    covered = [s for s in slugs if _found(s)]
...
# 6. Camelot Jedimaster — SCRATCHED
...
p("  R6: Triedtotellya — SCRATCHED. Manual form pick no longer applies.")
p()
p("NOTES PER RACE:")
for row in summary_rows:
    rn = row['race_no']
...
# ── Save output ───────────────────────────────────────────────────────────────
outpath = __file__.replace("launceston_22march2026_sim.py", "launceston_22march2026_sim.txt")
